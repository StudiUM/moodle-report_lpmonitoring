...
class report_lpmonitoring_external_testcase extends externallib_advanced_testcase {
    ...
    public function test_read_scale_configuration() {
        ...
        $result = external::read_report_competency_config($framework->get_id(), $scale->id);
        $result = (object) external_api::clean_returnvalue(external::read_report_competency_config_returns(), $result);

        $this->assertEquals($framework->get_id(), $result->competencyframeworkid);
    ...
    public function test_no_capability_to_create_scale_configuration() {
        ...
        try {
            external::create_report_competency_config($framework->get_id(), $scale->id, json_encode($scaleconfig));
    ...
    public function test_create_scale_configuration() {
        ...
        $result = external::create_report_competency_config($framework->get_id(), $scale->id, json_encode($scaleconfig));
    ...
    public function test_no_capability_to_update_scale_configuration() {
        ...
        try {
            external::update_report_competency_config($framework->get_id(), $scale->id,
    ...
    public function test_update_scale_configuration() {
        ...
        $result = external::update_report_competency_config($framework->get_id(), $scale->id,
                json_encode($scaleconfig));
        $result = external_api::clean_returnvalue(external::update_report_competency_config_returns(), $result);

        $this->assertTrue($result);

        $reportconfig = external::read_report_competency_config($framework->get_id(), $scale->id);
        $reportconfig = (object) external_api::clean_returnvalue(external::read_report_competency_config_returns(), $reportconfig);

        $this->assertEquals($reportconfig->competencyframeworkid, $framework->get_id());
    ...
    public function test_read_plan() {
        ...
        $plan1 = $lpg->create_plan(array('userid' => $user1->id, 'templateid' => $tpl->get_id()));
        $plan2 = $lpg->create_plan(array('userid' => $user2->id, 'templateid' => $tpl->get_id(),
                'status' => plan::STATUS_ACTIVE));
        $plan3 = $lpg->create_plan(array('userid' => $user3->id, 'templateid' => $tpl->get_id(),
                'status' => plan::STATUS_ACTIVE));
        $plan4 = $lpg->create_plan(array('userid' => $user1->id, 'status' => plan::STATUS_COMPLETE));

        // Some ratings for user2.
        $lpg->create_user_competency(array('userid' => $user2->id, 'competencyid' => $c1a->get_id(),
            'grade' => 1, 'proficiency' => 0));
        $lpg->create_user_competency(array('userid' => $user2->id, 'competencyid' => $c1c->get_id(),
            'grade' => 2, 'proficiency' => 1));

        // Some ratings for user3.
        $lpg->create_user_competency(array('userid' => $user3->id, 'competencyid' => $c1a->get_id(),
            'grade' => 2, 'proficiency' => 1));

        // Get plans urls.
        $plan1url = url::plan($plan1->get_id())->out(false);
        $plan2url = url::plan($plan2->get_id())->out(false);
        $plan3url = url::plan($plan3->get_id())->out(false);
        $plan4url = url::plan($plan4->get_id())->out(false);
        ...
        $result = external::read_plan($plan4->get_id(), 0);
        $result = external::clean_returnvalue(external::read_plan_returns(), $result);
        $this->assertEquals($plan4->get_id(), $result['plan']['id']);
        ...
        $result = external::read_plan($plan1->get_id(), $tpl->get_id());
        $result = external::clean_returnvalue(external::read_plan_returns(), $result);
        $this->assertEquals($plan1->get_id(), $result['plan']['id']);
        ...
        $result = external::read_plan($plan2->get_id(), $tpl->get_id());
        $result = external::clean_returnvalue(external::read_plan_returns(), $result);
        $this->assertEquals($plan2->get_id(), $result['plan']['id']);
        ...
        $result = external::read_plan($plan3->get_id(), $tpl->get_id());
        $result = external::clean_returnvalue(external::read_plan_returns(), $result);
        $this->assertEquals($plan3->get_id(), $result['plan']['id']);
        ...
        $result = external::read_plan(0, $tpl->get_id());
        $result = external::clean_returnvalue(external::read_plan_returns(), $result);
        $this->assertEquals($plan1->get_id(), $result['plan']['id']);
        $this->assertEquals($plan1->get_name(), $result['plan']['name']);
        $this->assertEquals($user1->id, $result['plan']['user']['id']);
    ...
    public function test_get_competency_detail() {
        ...
        $comp1 = $lpg->create_competency(array('competencyframeworkid' => $framework->get_id(),
            'parentid' => $comp0->get_id()));   // In C1, and C2.
        ...
        $lpg->create_evidence(array('usercompetencyid' => $uc->get_id(), 'note' => 'Note text',
        ...
        $result = external::get_competency_detail($u1->id, $comp1->get_id(), $plan->get_id());
        $result = (object) external_api::clean_returnvalue(external::get_competency_detail_returns(), $result);

        $this->assertEquals($result->competencyid, $comp1->get_id());
        ...
        $urluserevidence = url::user_evidence($ue1->get_id())->out(false);
        ...
        $urlcompetencyids = array($comp1->get_id());
    ...
    public function test_list_plan_competencies() {
        ...
        $plan = $lpg->create_plan(array('userid' => $user->id, 'templateid' => $tpl->get_id()));

        $uc1a = $lpg->create_user_competency(array('userid' => $user->id, 'competencyid' => $c1a->get_id(),
            'status' => user_competency::STATUS_IN_REVIEW, 'reviewerid' => $this->creator->id));
        $uc1c = $lpg->create_user_competency(array('userid' => $user->id, 'competencyid' => $c1c->get_id(),
            'grade' => 1, 'proficiency' => 0));
        $uc2b = $lpg->create_user_competency(array('userid' => $user->id, 'competencyid' => $c2b->get_id(),
            'grade' => 2, 'proficiency' => 1));
        ...
        $completedplan = $lpg->create_plan(array('userid' => $user->id, 'templateid' => $tpl->get_id(),
                'status' => plan::STATUS_COMPLETE));

        $uc1a = $lpg->create_user_competency_plan(array('userid' => $user->id, 'competencyid' => $c1a->get_id(),
                'planid' => $completedplan->get_id()));
        $uc1b = $lpg->create_user_competency_plan(array('userid' => $user->id, 'competencyid' => $c1c->get_id(),
                'planid' => $completedplan->get_id()));
        $uc2b = $lpg->create_user_competency_plan(array('userid' => $user->id, 'competencyid' => $c2b->get_id(),
    ...
    public function test_get_lp_monitoring_competency_statistics() {
        ...
        $plan = $lpg->create_plan(array('userid' => $u4->id, 'templateid' => $template->get_id(), 'status' => plan::STATUS_ACTIVE));
        ...
        $result = external::get_competency_statistics($comp1->get_id(), $template->get_id());
    ...
    public function test_search_templates() {
        ...
        $this->assertEquals($template2->get_id(), $result[0]['id']);
        $this->assertEquals($template1->get_id(), $result[1]['id']);
    ...
    public function test_get_plans_for_scale_values_in_plans() {
        ...
        $tpl = $lpg->create_template();
        $lpg->create_template_competency(array('templateid' => $tpl->get_id(), 'competencyid' => $comp1->get_id()));
        $lpg->create_template_competency(array('templateid' => $tpl->get_id(), 'competencyid' => $comp2->get_id()));
        $lpg->create_template_competency(array('templateid' => $tpl->get_id(), 'competencyid' => $comp3->get_id()));
        $lpg->create_template_competency(array('templateid' => $tpl->get_id(), 'competencyid' => $comp4->get_id()));
        ...
        $lpg->create_user_competency(array('userid' => $user1->id, 'competencyid' => $comp1->get_id(),
            'grade' => 1, 'proficiency' => 0));
        $lpg->create_user_competency(array('userid' => $user1->id, 'competencyid' => $comp2->get_id(),
            'grade' => 2, 'proficiency' => 1));

        // Some ratings for user2.
        $lpg->create_user_competency(array('userid' => $user2->id, 'competencyid' => $comp3->get_id(),
            'grade' => 2, 'proficiency' => 0));

        // Some ratings for user3.
        $lpg->create_user_competency_plan(array('userid' => $user3->id, 'competencyid' => $comp2->get_id(),
            'planid' => $plan3->get_id(), 'grade' => 3, 'proficiency' => 1));
        // Some ratings for user3.
        $lpg->create_user_competency_plan(array('userid' => $user3->id, 'competencyid' => $comp3->get_id(),
            'planid' => $plan3->get_id(), 'grade' => 3, 'proficiency' => 1));

        // Specify one scale value as filter.
        $scalevalues = '[{"scalevalue" : 2, "scaleid" :' . $framework->get_scaleid() . '}]';
        $scalefilterbycourse = 0;
        $result = external::read_plan(0, $tpl->get_id(), $scalevalues, $scalefilterbycourse);
        ...
        $scalevalues = '[{"scalevalue" : 1, "scaleid" :' . $framework->get_scaleid() . '}, '
                . '{"scalevalue" : 3, "scaleid" :' . $framework->get_scaleid() .'}]';
        $scalefilterbycourse = 0;
        $result = external::read_plan(0, $tpl->get_id(), $scalevalues, $scalefilterbycourse);
    ...
    public function test_get_plans_for_scale_values_in_courses() {
        ...
        $lpg->create_course_competency(array('competencyid' => $comp4->get_id(), 'courseid' => $course4->id));
        ...
        $record2->competencyid = $comp2->get_id();
        $record2->proficiency = 0;
        $record2->grade = 3;
        $record2->timecreated = 10;
        $record2->timemodified = 10;
